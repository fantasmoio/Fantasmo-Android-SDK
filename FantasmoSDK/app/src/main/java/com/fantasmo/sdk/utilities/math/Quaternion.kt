...
class Quaternion {
    ...
    operator fun set(axis: Vector3?, angle: Float) {
        Preconditions.checkNotNull(axis, "Parameter \"axis\" was null.")
        set(axisAngle(axis!!, angle))
    }

    /** Set each value and normalize the Quaternion  */
    operator fun set(qx: Float, qy: Float, qz: Float, qw: Float) {
        x = qx
        y = qy
        z = qz
        w = qw
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return if (this === other) {
            true
        } else equals(this, other as Quaternion?)
    }
    ...
    companion object {
         ...
         */
        fun lerp(a: Quaternion, b: Quaternion, ratio: Float): Quaternion {
   ...
   */
        fun slerp(start: Quaternion, end: Quaternion, t: Float): Quaternion {
            ...
            val orientation0: Quaternion = start.normalized()
            var orientation1: Quaternion = end.normalized()

            // cosTheta0 provides the angle between the rotations at t=0
            var cosTheta0: Double = dot(orientation0, orientation1).toDouble()
            ...
            val result: Quaternion = add(
                orientation0.scaled(s0.toFloat()), orientation1.scaled(
                    s1.toFloat()
                )
            )
        ...
        fun rotationBetweenVectors(start: Vector3, end: Vector3): Quaternion {
            ...
            val cosTheta = Vector3.dot(start, end)
        ...
        fun lookRotation(forwardInWorld: Vector3?, desiredUpInWorld: Vector3?): Quaternion {
            var desiredUpInWorld = desiredUpInWorld
            ...
            val rotateForwardToDesiredForward: Quaternion =
            ...
            val newUp: Vector3 = rotateVector(rotateForwardToDesiredForward, Vector3.up())
            val rotateNewUpToUpwards: Quaternion =
                rotationBetweenVectors(newUp, desiredUpInWorld)
